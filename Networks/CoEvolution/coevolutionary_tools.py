...
def synchronous_updating(graph,cooperators,defectors,S,T):
    ...
    for indiv in range(N):
        neigh_indiv=random.choice(list(graph[indiv].keys()))
        cooperators,defectors=strategy_update(graph,indiv,neigh_indiv,cooperators,defectors,S,T)
...
def simulation(S,T,W,N,z,f):
    ...
    while len(cooperators)<N and generations<(10**8):
        ...
        if W>0:
     #       print("asynchronous")
            graph,cooperators,defectors=asynchronous_updating(graph,cooperators,defectors,S,T)
        else:
      #      print("synchronous")
    ...
    if len(cooperators)!=N and generations>1000:
        percentage_of_cooperators=sum(fraction_of_cooperators_per_generation[-1000:])/1000
    else:
        percentage_of_cooperators=1.0
        
    return percentage_of_cooperators,heterogeneity(graph,N,z),k_max(graph),cumulative_degree_dist(graph,N)
...
def cumulative_degree_dist(graph,N):
    ...
    for key in degs.keys():
        cum_degs[key]=sum([degs.get(i) for i in range(key,k_max+1) if not degs.get(i)==None])/N
    return cum_degs
