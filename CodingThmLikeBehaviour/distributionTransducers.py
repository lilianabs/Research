...
def analyzeStringForTransducer(ro):
    if correctEncoding(ro):
        ...
        if isValidTransducer(bins,outs):
            ...
            return [1,sigma,p,len(bins)//2,computeTransducer(bins,outs,p),len(sigma)+len(p)]
        else:            
...
def runExperiment(n,m):
    ...
    csvWriter.writerow(['string', 'valid_encoding', 'sigma', 'string_p','num_states','output','output_complexity'])
    ...
    for ro in list_strings:
        ...
        if result[0]==1:        
            csvWriter.writerow([ro,result[0],result[1],result[2],result[3],result[4],result[5]])
